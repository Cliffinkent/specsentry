    ...
    const response = await this.client.responses.create(
      {
        ...
        instructions: [
          ...
          "If the criterion is subjective or lacks a measurable visible result, do not invent a requirement; create only the smallest observation plan needed for the evaluator to identify the missing clarification.",
          "If the supplied instructions define an unavailable prerequisite, verify that prerequisite once and stop rather than trying to bypass it.",
    ...
    > = [
      {
        type: "input_text",
        text: JSON.stringify({
          evaluationCaseId: input.input.evaluationCaseId || null,
          userStory: input.input.userStory,
          criterion: input.input.acceptanceCriteria,
          startingInstructions: input.input.startingInstructions,
    ...
    const response = await this.client.responses.create(
      {
        ...
        instructions: [
          ...
          "Return pass only when captured evidence demonstrates the measurable criterion, and fail only when captured evidence demonstrates an application defect against that criterion.",
          "Return blocked when a defined prerequisite is unavailable and downstream product behavior therefore cannot be judged. Explain the blocker and do not create a finding unless separate captured evidence demonstrates an application failure.",
          "Return inconclusive when the criterion lacks an observable or measurable expected result. Explain exactly what clarification is needed and never invent a product requirement.",
          "Create a finding only when status is fail. Missing price or required decision information, missing required delivery validation, and lost basket state are high severity.",
