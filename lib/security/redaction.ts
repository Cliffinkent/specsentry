...
const internalPath = /\/(?:app|Users|home|private|tmp|var)\/[A-Za-z0-9._~!$&'()+,;=:@%\-/]+/g;
...
export function redactSensitive(value: unknown): string {
  ...
  return (text || "Unknown error")
    ...
    .replace(internalPath, "[REDACTED_PATH]")
...
export function safeUserError(error: unknown, fallback = "SpecSentry could not complete that request.") {
  if (error instanceof Error && (error.name === "UrlPolicyError" || error.name === "PublicDemoRequestError")) {
